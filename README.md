...
また、各小選挙区の ID を `{都道府県ID}-{区番号}` と取り決め、存在の確認や並び替えを行うことができます。

```ts
isShuDistrictId('tokyo-30', '2026-01-01') // true
isShuDistrictId('tokyo-30', '2021-10-31') // false (2022年の区割り改定で新設されたため)

// compareShuDistrictIds は sort の比較関数として使用
